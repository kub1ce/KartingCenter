<x-admin-layout>
<div>
    <h1 class="text-2xl font-black text-white uppercase tracking-wider border-l-4 border-yellow-500 pl-4 mb-8">Редактирование новости</h1>
                ...
                <form action="{{ route('admin.news.update', $news->id) }}" method="POST" id="newsForm">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="content" id="content" rows="6" 
                                  class="w-full bg-black/50 border border-white/10 text-gray-300 py-3 px-4 rounded-md focus:border-lime-500 focus:ring-0 focus:outline-none transition text-sm leading-relaxed" oninput="updatePreview()">{{ old('content', $news->content) }}</textarea>
                    </div>
                    ...
                    <div class="mb-6 flex items-center">
                        <input type="checkbox" name="is_published" value="1" id="is_published" class="w-5 h-5 rounded bg-black border-gray-700 text-lime-500 focus:ring-lime-500 focus:ring-offset-black cursor-pointer" @checked(old('is_published', $news->is_published))>
                        <label for="is_published" class="ml-3 text-gray-400 text-sm font-bold uppercase tracking-wider cursor-pointer">Опубликована</label>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-white/5">
                        ...
                        <a href="{{ route('admin.news.index') }}" class="text-gray-600 hover:text-white text-xs font-bold uppercase tracking-widest transition">Отмена</a>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="w-full lg:w-1/2">
            ...
            <div class="glass-card rounded-xl overflow-hidden group">
                <div class="h-48 bg-black/50 overflow-hidden border-b border-white/5 relative">
                    <div id="preview-image-container" class="w-full h-full flex items-center justify-center text-gray-600 bg-gray-900/50 @if($news->image_url) hidden @endif">
                        <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    </div>
                    <img id="preview-image" src="{{ $news->image_url }}" alt="" class="w-full h-full object-cover @if(!$news->image_url) hidden @endif absolute inset-0">
                </div>
                
                <div class="p-6">
                    <p class="text-xs text-lime-400 font-bold uppercase tracking-widest mb-2 font-mono">Сегодня</p>
                    <h2 id="preview-title" class="text-lg font-black text-white uppercase mb-3 tracking-wide">{{ $news->title }}</h2>
                    <p id="preview-content" class="text-gray-500 text-sm leading-relaxed">{{ $news->content }}</p>
                </div>
            </div>
        </div>
    </div>
