<x-admin-layout>
<div>
    <h1 class="text-xl md:text-2xl font-black text-white uppercase tracking-wider border-l-4 border-yellow-500 pl-4 mb-8">Редактирование акции</h1>

    <div class="flex flex-col lg:flex-row gap-8" x-data="{ title: '{{ old('title', $promotion->title) }}', description: `{{ old('description', $promotion->description) }}`, discount: {{ old('discount_percent', $promotion->discount_percent) }}, startDate: '{{ old('start_date', $promotion->start_date->format('Y-m-d')) }}', endDate: '{{ old('end_date', $promotion->end_date->format('Y-m-d')) }}' }">
            ...
            <div class="glass-card p-6 rounded-xl">
                <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="title" id="title" x-model="title" value="{{ old('title', $promotion->title) }}" 
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="description" id="description" rows="4" x-model="description"
                                  class="w-full bg-black/50 border border-white/10 text-gray-300 py-3 px-4 rounded-md focus:border-red-500 focus:ring-0 focus:outline-none transition text-sm leading-relaxed">{{ old('description', $promotion->description) }}</textarea>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            <input type="number" name="discount_percent" id="discount_percent" min="1" max="99" x-model="discount" value="{{ old('discount_percent', $promotion->discount_percent) }}" 
                    ...
                    <div class="flex gap-4 mb-5">
                        <div class="w-1/2">
                            ...
                            <input type="date" name="start_date" id="start_date" x-model="startDate" value="{{ old('start_date', $promotion->start_date->format('Y-m-d')) }}" 
                        ...
                        <div class="w-1/2">
                            ...
                            <input type="date" name="end_date" id="end_date" x-model="endDate" value="{{ old('end_date', $promotion->end_date->format('Y-m-d')) }}" 
                                   class="w-full bg-black/50 border border-white/10 text-white py-3 px-4 rounded-md focus:border-red-500 focus:ring-0 focus:outline-none transition text-sm font-mono" style="color-scheme: dark;">
                    ...
                    <div class="mb-6 flex items-center">
                        <input type="checkbox" name="is_active" value="1" id="is_active" class="w-5 h-5 rounded bg-black border-gray-700 text-lime-500 focus:ring-lime-500 focus:ring-offset-black cursor-pointer" @checked(old('is_active', $promotion->is_active))>
                        <label for="is_active" class="ml-3 text-gray-400 text-sm font-bold uppercase tracking-wider cursor-pointer">Активна</label>
                    </div>
